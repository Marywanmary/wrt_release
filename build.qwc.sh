...
if [[ -d $TARGET_DIR ]]; then
    ...
    # .config, config.buildinfo, Packages.manifest 也被加入删除范围
...
# --- START: 复制编译产物并处理重命名 ---
# 复制所有编译产物文件（包括固件和manifest）从TARGET_DIR到FIRMWARE_DIR
...
if [[ -d "$IPK_DIR" ]]; then
    # 创建设备专属的ipk目录
    mkdir -p "$FIRMWARE_DIR/ipk/$Dev"
    # 查找所有ipk文件并复制
    find "$IPK_DIR" -name "*.ipk" -type f -exec cp -f {} "$FIRMWARE_DIR/ipk/$Dev/" \;
    echo "Copied ipk files for $Dev"
fi

# --- START: 复制APK文件 ---
APK_DIR="$BASE_PATH/$BUILD_DIR/bin/apk"  # 请根据实际路径修改
if [[ -d "$APK_DIR" ]]; then
    mkdir -p "$FIRMWARE_DIR/apk/$Dev"
    find "$APK_DIR" -name "*.apk" -type f -exec cp -f {} "$FIRMWARE_DIR/apk/$Dev/" \;
    echo "Copied apk files for $Dev"
fi

# === 新增部分：固件重命名和配置文件重命名 ===
# 解析设备名称，检查是否符合三段式结构 (chip_branch_config)
if [[ $Dev =~ ^([^_]+)_([^_]+)_([^_]+)$ ]]; then
    CHIP="${BASH_REMATCH[1]}"      # 芯片部分 (如 ipq60xx)
    BRANCH_ABBR="${BASH_REMATCH[2]}" # 分支缩写 (如 immwrt 或 libwrt)
    CONFIG="${BASH_REMATCH[3]}"     # 配置部分 (如 Pro, Max, Ultra)

    echo "Device name parsed: CHIP=$CHIP, BRANCH_ABBR=$BRANCH_ABBR, CONFIG=$CONFIG"

    # --- 重命名固件文件 ---
    # 针对 libwrt 设备，根据文件名模式提取模型和模式
    # 例如：libwrt-qualcommax-ipq60xx-jdcloud_re-ss-01-squashfs-factory.bin
    # 期望的格式：libwrt-re-ss-01-factory-Pro.bin
    # 对于 immwrt 设备，使用相同的模式
    # 例如：immortalwrt-qualcommax-ipq60xx-jdcloud_re-ss-01-squashfs-factory.bin
    # 期望的格式：immwrt-re-ss-01-factory-Pro.bin
    for firmware in "$FIRMWARE_DIR"/*.bin; do
        # 获取文件名（不含路径）
        filename=$(basename "$firmware")

        # 检查文件名是否匹配 libwrt 或 immortalwrt 模式
        # 注意：这里假设文件名中包含 "libwrt-" 或 "immortalwrt-" 前缀
        # 并且包含 "jdcloud_" 之后的部分是模型名
        # 这个正则表达式更通用一些，适用于两种情况
        if [[ $filename =~ ^(libwrt|immortalwrt)-.*-jdcloud_(.+)-squashfs-(.+)\.bin ]]; then
            PREFIX="${BASH_REMATCH[1]}"   # "libwrt" 或 "immortalwrt"
            MODEL="${BASH_REMATCH[2]}"    # 模型 (如 re-ss-01)
            MODE="${BASH_REMATCH[3]}"     # 模式 (如 factory, sysupgrade)
            # 构建新文件名
            # 如果是 libwrt 设备，则使用 libwrt 前缀
    ...
    for manifest_file in "$FIRMWARE_DIR"/*.manifest; do
        if [[ -f "$manifest_file" ]]; then
            ...
            # 重命名文件
            mv "$manifest_file" "$FIRMWARE_DIR/$new_filename"
            echo "Renamed manifest file $filename to $new_filename"
        fi
    done

    # --- 重命名配置文件 ---
    # 注意：这里的 .config, config.buildinfo 是在 TARGET_DIR/bin/targets 下的
    # 但它们会被复制到 FIRMWARE_DIR，所以需要在 FIRMWARE_DIR 中处理
    config_files=(".config" "config.buildinfo")
    for file in "${config_files[@]}"; do
        if [[ -f "$FIRMWARE_DIR/$file" ]]; then
            # 构建新文件名
            # 保持原有的点号前缀，然后加上芯片、分支、配置信息
...
# --- START: 复制到临时目录供工作流上传 ---
TEMP_OUTPUT_DIR="$BASE_PATH/build_output_$Dev"
\rm -rf "$TEMP_OUTPUT_DIR"
mkdir -p "$TEMP_OUTPUT_DIR"

# 复制所有最终产物到临时目录
# 包括 bin, manifest, config, ipk, apk
find "$FIRMWARE_DIR" -type f \( -name "*.bin" -o -name "*.manifest" -o -name "*efi.img.gz" -o -name "*.itb" -o -name "*.fip" -o -name "*.ubi" -o -name "*rootfs.tar.gz" -o -name ".config" -o -name "config.buildinfo" -o -name "Packages.manifest" \) -exec cp -f {} "$TEMP_OUTPUT_DIR/" \;

# 复制 ipk 文件
if [[ -d "$FIRMWARE_DIR/ipk/$Dev" ]]; then
    mkdir -p "$TEMP_OUTPUT_DIR/ipk/"
    find "$FIRMWARE_DIR/ipk/$Dev" -type f -name "*.ipk" -exec cp -f {} "$TEMP_OUTPUT_DIR/ipk/" \;
fi

# 复制 apk 文件
if [[ -d "$FIRMWARE_DIR/apk/$Dev" ]]; then
    mkdir -p "$TEMP_OUTPUT_DIR/apk/"
    find "$FIRMWARE_DIR/apk/$Dev" -type f -name "*.apk" -exec cp -f {} "$TEMP_OUTPUT_DIR/apk/" \;
fi
...
echo "Build completed successfully for $Dev. Final outputs are in $TEMP_OUTPUT_DIR"
