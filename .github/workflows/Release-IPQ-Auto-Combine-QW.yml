...
# --- 创建固件输出目录 ---
FIRMWARE_DIR="$BASE_PATH/firmware/$Dev"
...
PREFIX=""
SUFFIX=""
MODEL_TYPE="" # immwrt or libwrt or openwrt
case "$Dev" in
    ...
    *openwrt_Pro) PREFIX="openwrt-re"; SUFFIX="Pro" ;;
    *openwrt_Max) PREFIX="openwrt-re"; SUFFIX="Max" ;;
    *openwrt_Ultra) PREFIX="openwrt-re"; SUFFIX="Ultra" ;;
...
for file in "$FIRMWARE_DIR"/*.bin; do
    if [ -f "$file" ]; then
        ...
        if [[ "$original_name" == *_re-* ]] || [[ "$original_name" == *-re-* ]]; then
            # 提取设备型号部分 (如 cs-02 或 ss-01)
            # 使用更精确的正则表达式匹配设备型号
            if [[ "$original_name" =~ (_re-|-re-)([a-zA-Z]+-[0-9]+) ]]; then
                part="${BASH_REMATCH[2]}"
                log "Extracted device model part: $part"
                ...
                warn "Could not extract device model from $original_name. Keeping original."
            fi
        else
