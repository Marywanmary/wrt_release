...
disable_hostapd_he_mu_edca() {
    local hostapd_mk="$BUILD_DIR/package/network/services/hostapd/Makefile"
    
    if [ -f "$hostapd_mk" ]; then
        echo "禁用hostapd的HE MU EDCA功能..."
        # 禁用HE MU EDCA相关配置
        sed -i 's/CONFIG_HE_MU_EDCA=y/# CONFIG_HE_MU_EDCA is not set/g' "$hostapd_mk"
        # 确保CONFIG_HE选项仍然启用
        if ! grep -q "CONFIG_HE=y" "$hostapd_mk"; then
            sed -i '/CONFIG_IEEE80211AX=y/a CONFIG_HE=y' "$hostapd_mk"
        fi
    fi
}
main() {
