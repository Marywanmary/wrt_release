...
update_feeds() {
    ...
    # 检查并添加 netspeedtest/taskplan 源
    if ! grep -q "netspeedtest" "$BUILD_DIR/$FEEDS_CONF"; then
        # 确保文件以换行符结尾
        [ -z "$(tail -c 1 "$BUILD_DIR/$FEEDS_CONF")" ] || echo "" >>"$BUILD_DIR/$FEEDS_CONF"
        echo "src-git netspeedtest https://github.com/sirpdboy/luci-app-netspeedtest" >>"$BUILD_DIR/$FEEDS_CONF"
    fi

    if ! grep -q "taskplan" "$BUILD_DIR/$FEEDS_CONF"; then
        # 确保文件以换行符结尾
        [ -z "$(tail -c 1 "$BUILD_DIR/$FEEDS_CONF")" ] || echo "" >>"$BUILD_DIR/$FEEDS_CONF"
